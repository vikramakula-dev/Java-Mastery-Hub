export const wrapperClassesData = {
  ...
  examples: [
    ...
    { level: "Intermediate", title: "Wrapper Class Caching", code: "Integer a = 100;\nInteger b = 100;\nSystem.out.println(a == b);\n\nInteger x = 200;\nInteger y = 200;\nSystem.out.println(x == y);", output: "true\nfalse", explanation: "Integer caches values between -128 and 127. Values outside this range create new objects, so == evaluates to false.", selenium: "A count assertion using == can pass with small numbers in dev and fail with real production-sized numbers — use .equals().",
      walkthrough: [
        { code: "Integer a = 100;\nInteger b = 100;", note: "Autoboxing calls Integer.valueOf(100) behind the scenes, and valueOf() serves values from -128 to 127 out of a pre-built cache — so a and b receive the SAME cached object." },
        { code: "System.out.println(a == b);", note: "true — but only because both references point at the one cached object. This 'working' == is a trap: it teaches the wrong habit that fails the moment values grow." },
        { code: "Integer x = 200;\nInteger y = 200;", note: "200 is outside the cache range, so each autoboxing creates a brand-new Integer object — two objects, same value." },
        { code: "System.out.println(x == y);", note: "false — same value, different objects. The lesson: == on wrappers is value-comparison ONLY by accident within the cache range. Always .equals() for wrapper content comparison." }
      ]
    },
    ...
    { level: "Selenium-Oriented", title: "Extracting integers for assertions", code: "String resultText = \"You have 5 unread messages.\";\nString countStr = resultText.replaceAll(\"[^0-9]\", \"\");\nint count = Integer.parseInt(countStr);\nSystem.out.println(count > 0);", output: "true", explanation: "Extracting only digits from a string and converting to int.", selenium: "Asserting numeric values present within dynamic text blocks.",
      walkthrough: [
        { code: "resultText.replaceAll(\"[^0-9]\", \"\")", note: "replaceAll takes a REGEX (unlike replace, which is literal). [^0-9] means 'any character that is NOT a digit' — so this deletes everything except digits, leaving \"5\". Brutally effective, but beware: 'Showing 1 of 500' would collapse to \"1500\" — use split-based extraction when multiple numbers exist." },
        { code: "Integer.parseInt(countStr)", note: "Converts the cleaned digits into a primitive int. If the regex left an empty string (no digits at all in the text), this throws NumberFormatException — worth guarding when the text can legitimately contain zero numbers." },
        { code: "count > 0", note: "Now it's a real number, so real assertions work: >, <, ==, arithmetic. Comparing the raw STRING \"5\" > \"10\" would be lexicographic and wrong — this is why the conversion step exists at all." }
      ]
    }
  ],
  interview: [
